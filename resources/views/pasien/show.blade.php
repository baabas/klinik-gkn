...
{{-- PERUBAHAN: Menggunakan variabel $pasien --}}
...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body p-4">
        {{-- PERUBAHAN: Logika untuk menampilkan data karyawan atau non-karyawan --}}
        <div class="row">
            <div class="col-md-6">
                <p><strong>Nama:</strong><br> {{ $pasien->nama_karyawan }}</p>
                
                {{-- Cek jika pasien adalah karyawan --}}
                @if($pasien->karyawan)
                    <p><strong>NIP:</strong><br> {{ $pasien->nip }}</p>
                    <p><strong>Tanggal Lahir:</strong><br>
                        {{ $pasien->karyawan->tanggal_lahir ? \Carbon\Carbon::parse($pasien->karyawan->tanggal_lahir)->translatedFormat('d F Y') : '-' }}
                    </p>
                    <p class="mb-md-0"><strong>Usia:</strong><br>
                        {{ $pasien->karyawan->tanggal_lahir ? \Carbon\Carbon::parse($pasien->karyawan->tanggal_lahir)->age . ' Tahun' : '-' }}
                    </p>
                {{-- Cek jika pasien adalah non-karyawan --}}
                @elseif($pasien->nonKaryawan)
            ...
            <div class="col-md-6">
                 @if($pasien->karyawan)
                    <p><strong>Kantor:</strong><br> {{ $pasien->karyawan->kantor ?? '-' }}</p>
                    <p class="mb-0"><strong>Alamat:</strong><br> {{ $pasien->karyawan->alamat ?? '-' }}</p>
                @elseif($pasien->nonKaryawan)
    ...
    <div class="card-header bg-light d-flex flex-wrap justify-content-between align-items-center">
        ...
        <div class="mt-2 mt-md-0">
            @if(Auth::user()->hasRole('DOKTER'))
                <div class="btn-group">
                    {{-- PERUBAHAN: Menggunakan $pasien dan identifier generik --}}
                    <a href="{{ route('rekam-medis.create', $pasien->nip ?? $pasien->nik) }}" class="btn btn-success btn-sm">
                        <i class="bi bi-plus-circle"></i> Rekam Medis Baru
                    </a>
                    <a href="{{ route('checkup.create', $pasien->nip ?? $pasien->nik) }}" class="btn btn-info btn-sm text-white">
                                        ...
                                        @if($rekam->resepObat->isNotEmpty())
                                            ...
                                            <ul class="list-unstyled list-resep">
                                                @foreach($rekam->resepObat as $resep)
                                                    {{-- Asumsi relasi ke barang medis, bukan obat --}}
                                                    <li><i class="bi bi-prescription me-2"></i>{{ $resep->barangMedis->nama_barang ?? 'N/A' }} <span class="badge bg-secondary rounded-pill">{{ $resep->jumlah }}</span></li>
                                                @endforeach
                                            </ul>
                                        @endif
                                        @if($rekam->terapi)
