...
class LaporanViewController extends Controller
{
    ...
    public function daftarPenyakit(Request $request): View
    {
        $filter = $this->getFilterBulanTahun($request);
        $user = Auth::user();
        ...
        $daftar_penyakit = DB::table('detail_diagnosa as dd')
            ...
            ->whereYear('rm.tanggal_kunjungan', $filter['tahun'])
            ->whereMonth('rm.tanggal_kunjungan', $filter['bulan'])
            ->select('dp.ICD10', 'dp.nama_penyakit')
            ->distinct()
            ->orderBy('dp.nama_penyakit')
            ->get();
        ...
        $data_kasus = DB::table('detail_diagnosa as dd')
            ...
            ->whereMonth('rm.tanggal_kunjungan', $filter['bulan'])
            ->select('dp.nama_penyakit', DB::raw('DAY(rm.tanggal_kunjungan) as hari'), DB::raw('COUNT(dd.id_detail_diagnosa) as jumlah'))
            ->groupBy('dp.nama_penyakit', 'hari')
            ->get()
            ->groupBy('nama_penyakit');

        return view('laporan.penyakit', compact('daftar_penyakit', 'data_kasus', 'filter'));
    }

    public function daftarKunjungan(Request $request): View
    {
        $filter = $this->getFilterBulanTahun($request);
        $user = Auth::user();
        $idLokasi = $user->id_lokasi;
        ...
        return view('laporan.kunjungan', compact('daftar_kantor', 'data_kunjungan', 'filter'));
    }
